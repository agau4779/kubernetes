...
func (cont *GCEIngressController) BackendServiceUsingNEG(svcPorts []v1.ServicePort) (bool, error) {
...
func (cont *GCEIngressController) BackendServiceUsingIG(svcPorts []v1.ServicePort) (bool, error) {
	return cont.backendMode(svcPorts, "instanceGroups")
}

func (cont *GCEIngressController) backendMode(svcPorts []v1.ServicePort, keyword string) (bool, error) {
	gceCloud := cont.Cloud.Provider.(*gcecloud.GCECloud)
	beList, err := gceCloud.ListGlobalBackendServices()
	if err != nil {
	...
	for _, bs := range beList {
		...
		for _, sp := range svcPorts {
			...
			negString := cont.UID + cont.Ns + sp.Name + sp.TargetPort.String()
			negHash := fmt.Sprintf("%x", sha256.Sum256([]byte(negString)))[:8]
			Logf("UID %v, ns %v, name %v, targetport %v, hash %v", cont.UID, cont.Ns, sp.Name, sp.TargetPort.String(), negHash)
			Logf("bsname: %v", bs.Name)
...
// Current GCE ingress controller allows traffic to the default HTTP backend
// by default, so retrieve its nodePort if includeDefaultBackend is true.
func (j *IngressTestJig) GetServicePorts(includeDefaultBackend bool) []v1.ServicePort {
	svcPorts := []v1.ServicePort{}
	if includeDefaultBackend {
		defaultSvc, err := j.Client.CoreV1().Services(metav1.NamespaceSystem).Get(defaultBackendName, metav1.GetOptions{})
		Expect(err).NotTo(HaveOccurred())
		svcPorts = append(svcPorts, defaultSvc.Spec.Ports[0])
	}
	...
	for _, svcName := range backendSvcs {
		...
		Expect(err).NotTo(HaveOccurred())
		svcPorts = append(svcPorts, svc.Spec.Ports[0])
	}
	return svcPorts
}
